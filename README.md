...
다른 플러그인의 `%플레이스홀더%` 는 색을 **열어 두고 닫지 않는** 경우가 많습니다.
`%cmi_user_prefix%` 가 `<dark_green>중` 을 돌려주면 그 뒤의 칭호·닉네임까지 전부 초록이 됩니다.

기본값 `display.color-bleed: false` 는 각 `%토큰%` 을 가둬 이걸 막습니다.

```yaml
display:
  color-bleed: false   # false(기본) = 색이 뒤로 새지 않음 / true = 예전처럼 흘려보냄
```

접두사 색을 이름 전체에 입히고 싶다면 `true` 로 바꾸세요.
설정과 무관하게 `<reset>` 이나 `&r` 을 직접 넣어 원하는 지점에서 끊을 수도 있습니다.

> 토큰 **바깥**에서 열어 둔 서식은 그대로 유지됩니다.
> `<gray>이름: %토큰% 님` 에서 " 님" 은 계속 회색입니다
> (`<reset>` 을 쓰면 이 회색까지 지워지기 때문에 그 방식을 쓰지 않습니다).
...
### 다른 플러그인 명령어에서 닉네임 쓰기

`nickname.command-bridge` 를 켜면 (기본 켜짐) 다른 플러그인 명령어에서도 닉네임으로 사람을
지목할 수 있습니다. 아이디가 `ninesik` 인 사람이 닉네임을 `나인` 으로 바꿨다면:

| 설정 | 하는 일 |
|---|---|
| `suggest: true` | 플레이어 이름을 제안하는 자리에 `나인` 도 함께 뜹니다 |
| `resolve: true` | `나인` 으로 친 인자를 실행 직전에 `ninesik` 으로 되돌립니다 |

`resolve` 가 꺼져 있으면 `suggest` 는 보기용에 그칩니다 — 다른 플러그인은 `나인` 이라는
이름을 모르기 때문에 "플레이어를 찾을 수 없습니다" 로 실패합니다.

**닉네임이 필요 없는 자리에는 뜨지 않습니다.** 서버가 그 자리에 이미 실제 아이디를 제안하고
있는지를 보고 판단하므로, `/it create <종류>` 처럼 플레이어 이름이 아닌 자리는 건드리지 않습니다.

> **바닐라 명령어(`/msg`, `/tp` 등)의 제안은 예외입니다.** 플레이어 이름 후보를 서버가 아니라
> 클라이언트가 자기 목록에서 직접 만들기 때문에 서버가 끼어들 수 없습니다. 다만 닉네임을
> **직접 쳐서** 쓰는 것은 바닐라 명령어에서도 됩니다(`resolve` 가 켜져 있을 때).

대상은 **접속 중인 사람**뿐입니다. 오프라인 대상까지 닉네임으로 찾으려면 명령어를 칠 때마다
DB 를 뒤져야 해서 넣지 않았습니다.

`resolve` 는 안전을 위해 다음을 지킵니다.

- **맨 앞에서 일치하는 인자 하나만** 바꿉니다. `/msg 나인 나인 안녕` 의 본문까지 바뀌지 않습니다.
- 실제 아이디가 그대로 들어오면 손대지 않습니다. 아이디로 지목한 쪽이 항상 우선입니다.
- `unique: false` 로 같은 닉네임을 여럿이 쓰고 있으면 그 닉네임은 치환하지 않습니다
  (대상이 한 명으로 확정되지 않으므로).
- 공백이 들어간 닉네임(`allow-space: true`)은 한 인자로 칠 수 없어 제외됩니다.
- 닉네임과 똑같은 단어를 인자로 받는 명령어가 있다면 `excluded-commands` 에 적어 빼세요.

`/it` 자신은 치환 대상이 아닙니다. `/it create title 나인` 의 칭호 ID 처럼 플레이어가 아닌
자리에도 닉네임과 같은 값이 정상적으로 들어오기 때문입니다. 대신 `/it` 은 **대상 자리에서만**
닉네임을 직접 알아듣습니다 (`/it give 나인 title vip` 처럼).
...
엔티티는 최대 2개입니다. 내용이 바뀔 때만 다시 보냅니다.
퇴장·종료 시 제거되고, 기동 시 태그 기준으로 유령 엔티티를 청소합니다.

> **치장 플러그인과 겹치지 않습니다.** 이름표를 플레이어에 **태우지(passenger) 않고**
> 독립 엔티티로 따라다니게 합니다. 승객으로 태우면 HMCCosmetics·CMI 같은 치장 플러그인이
> 붙인 승객과 **세로로 쌓여** 이름표가 뜨거나 치장 위치가 밀리고, 승객이 있는 플레이어는
> 차원 이동에도 제약을 받습니다. 승객 목록을 아예 건드리지 않으므로 두 문제가 함께 사라집니다.
>
> 위치는 서버의 엔티티 트래커가 동기화합니다 — 추적 범위 안의 뷰어에게만, 전체 좌표가 아닌
> **델타**로 나갑니다(몹 한 마리와 같은 비용). 서 있으면 아예 전송하지 않고,
> `teleportDuration` 보간 덕분에 갱신 주기를 늘려도 부드럽게 보입니다.
