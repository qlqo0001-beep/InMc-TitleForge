...
class PlayerProfile(val uuid: UUID, name: String) {
    ...
    /**
     * 칭호/인장 ID 가 바뀌었을 때 보유 기록과 장착 슬롯을 새 ID 로 옮긴다.
     *
     * 이미 새 ID 를 보유 중이면 더 이른 획득 시각을 남긴다(둘 다 유지할 수는 없으므로).
     *
     * @return 보유 중이거나 장착 중이어서 실제로 바뀐 게 있으면 true.
     */
    fun renameBadge(type: BadgeType, oldId: String, newId: String): Boolean {
        var changed = false

        if (ownedSets.getValue(type).remove(oldId)) {
            changed = true
            val oldKey = key(type, oldId)
            val newKey = key(type, newId)
            val alreadyOwned = !ownedSets.getValue(type).add(newId)

            val movedObtained = obtainedTimes.remove(oldKey)
            if (movedObtained != null) {
                val existing = if (alreadyOwned) obtainedTimes[newKey] else null
                obtainedTimes[newKey] = if (existing == null) movedObtained else minOf(existing, movedObtained)
            }
            val movedExpiry = expiryTimes.remove(oldKey)
            if (movedExpiry != null && !alreadyOwned) expiryTimes[newKey] = movedExpiry
            recomputeNextExpiry()
        }

        for (slot in EquipSlot.entries) {
            if (slot.type == type && equipped(slot) == oldId) {
                setEquipped(slot, newId)
                changed = true
            }
        }
