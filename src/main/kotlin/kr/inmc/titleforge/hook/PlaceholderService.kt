...
class PlaceholderService(private val logger: Logger) {
    ...
    fun apply(player: Player, input: String): String {
        ...
        return runCatching { method.invoke(null, player, input) as? String ?: input }
            .getOrElse { error ->
                // 확장 하나가 던진 예외 때문에 치환 전체를 꺼버리면 안 된다.
                // PlaceholderAPI 자체가 사라진 경우(링크 오류)에만 영구히 중단한다.
                val cause = (error as? java.lang.reflect.InvocationTargetException)?.targetException ?: error
                if (cause is LinkageError || cause is ReflectiveOperationException) {
                    broken = true
                    logger.warning("PlaceholderAPI 연결이 끊겨 외부 치환을 끕니다: ${cause.message}")
                } else {
                    logger.warning("플레이스홀더 치환 실패: ${cause.message}")
                }
                input
            }
    }
