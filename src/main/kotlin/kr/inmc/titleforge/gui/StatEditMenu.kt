 ...
 * 장착 · 보유 스텟 편집 창.
 *
 * - **한 번에 한 종류**(바닐라/MMOItems/커스텀)만 다룬다. 종류 선택은 [StatKindMenu] 가 한다.
...
class StatEditMenu(
    ...
    private val kind: StatKind,
) : Menu(plugin, viewer, rows = StatLayout.ROWS) {

    private var page = 0

    /** 이 창이 다루는 종류만 남긴다. 배치 계산 자체는 [StatLayout] 이 그대로 담당한다. */
    private fun statsOf(category: StatCategory): List<Stat> =
        plugin.stats.byCategory(category).filter { it.kind == kind }

    private fun layout() = StatLayout.compute(page) { statsOf(it) }
    ...
    private fun renderHeader(badge: Badge) {
        button(StatLayout.SLOT_BACK, Gui.item(plugin, "gui.button.back", Material.OAK_DOOR)) {
            StatKindMenu(plugin, viewer, badgeId).openLater()
        }
    ...
    private fun renderCategories(badge: Badge, layout: StatLayout.Layout) {
        for ((slot, label) in layout.labels) {
            button(
                slot,
                Gui.categoryLabel(
                    plugin, label.category, statsOf(label.category),
                    badge.equipStats, badge.ownStats, label.continuation,
                ),
            )
        }
        ...
        button(
            ...
            Gui.item(
                ...
                placeholders = arrayOf(
                    ...
                    "max" to layout.pageCount,
                    "kind" to kind.display,
                    "total" to plugin.stats.ofKind(kind).size,
                ),
            ),
        )
