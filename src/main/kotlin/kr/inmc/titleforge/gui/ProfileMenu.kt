...
class ProfileMenu(
    ...
    override fun render() {
        ...
        button(
            13,
            // target 은 항상 온라인(이 메뉴는 온라인 플레이어만 대상으로 한다)이므로
            // 토큰 치환기에 넘길 수 있다. 빠뜨리면 %cmi_user_prefix% 같은 태그가 그대로 노출된다.
            Items.head(target, display.nameplate(profile, target.name, target), nicknameLore),
        ) {
            if (self && plugin.settings.nickname.enabled) {
