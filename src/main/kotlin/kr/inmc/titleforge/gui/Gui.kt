...
object Gui {
    ...
    fun badgeIcon(
        ...
        /** true면 좌클릭/Shift+우클릭 안내를 덧붙인다. 클릭 핸들러가 없는 미리보기 슬롯에서는 false로 둔다. */
        clickable: Boolean = true,
        /** 로어 맨 뒤에 덧붙일 줄. 창마다 다른 조작 안내를 넣을 때 쓴다. */
        extraLore: List<Component> = emptyList(),
    ): ItemStack {
        ...
        when {
            adminMode -> {
                if (clickable) lore += messages.component("gui.lore.admin-hint")
     ...
     *
     * @param stats 지금 화면에 보이는 스텟들. 종류별로 걸러 보여줄 때 전체 목록을 세면
     *   "설정됨 2 / 9" 처럼 화면과 맞지 않는 숫자가 나오므로 호출자가 넘겨준다.
     */
    fun categoryLabel(
        plugin: TitleForgePlugin,
