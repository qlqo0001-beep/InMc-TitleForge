...
class AdminMenu(
    ...
    override fun render() {
        ...
        list.drop(page * pageSize).take(pageSize).forEachIndexed { index, listed ->
            // 클릭 시점에는 ID 로 다시 찾는다. 그 사이 다른 관리자가 이름을 바꿨거나 지웠을 수 있다.
            val badgeType = listed.type
            val badgeId = listed.id
            button(
                index,
                Gui.badgeIcon(plugin, null, listed, owned = true, locked = false, equippedSlots = emptyList(), adminMode = true),
            ) { event ->
        ...
        button(navRow + 6, Gui.item(plugin, "gui.button.create", Material.WRITABLE_BOOK, placeholders = arrayOf("type" to type.display))) {
            ...
            plugin.dialogInput.prompt(viewer, "new_id", prompt) { input ->
                if (input == null) {
